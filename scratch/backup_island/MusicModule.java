 ...
 * Implements minimal, compact, and expanded views. Incorporates GNOME Adwaita
 * symbolic PNG icons, fixed click detection bounds, slide-up controls animation,
 * and classic "vines" EQ visualizer.
 */
...
public class MusicModule implements IslandModule {
    ...
    // Pre-loaded Adwaita Symbolic Icons (Recolored to white)
    ...
    private static final Identifier ICON_SHUFFLE = Identifier.of("glassmenu", "textures/gui/symbolic_icons/shuffle.png");
    private static final Identifier ICON_CONSECUTIVE = Identifier.of("glassmenu", "textures/gui/symbolic_icons/consecutive.png");
    private static final Identifier ICON_REPEAT = Identifier.of("glassmenu", "textures/gui/symbolic_icons/repeat.png");
    private static final Identifier ICON_REPEAT_SONG = Identifier.of("glassmenu", "textures/gui/symbolic_icons/repeat_song.png");
    private static final Identifier ICON_MAIL_FORWARD = Identifier.of("glassmenu", "textures/gui/symbolic_icons/mail_forward.png");
    ...
    private void drawSymbolicButton(DrawContext context, float x, float y, String iconName, float cx, float cy, float btnSize, float iconSize, float alpha, float scale, float hoverProgress, float widgetAlpha) {
        ...
        if ("media-seek-backward".equals(iconName)) {
            ...
            texture = ICON_PAUSE;
        } else if ("media-playlist-shuffle-symbolic".equals(iconName)) {
            texture = ICON_SHUFFLE;
        } else if ("media-playlist-consecutive-symbolic".equals(iconName)) {
            texture = ICON_CONSECUTIVE;
        } else if ("media-playlist-repeat-symbolic".equals(iconName)) {
            texture = ICON_REPEAT;
        } else if ("media-playlist-repeat-song-symbolic".equals(iconName)) {
            texture = ICON_REPEAT_SONG;
        } else if ("mail-forward".equals(iconName)) {
            texture = ICON_MAIL_FORWARD;
        }

        if (texture != null) {
            // Colors: standard white/gray or vibrant green for enabled shuffle/loop
            int iconColor = 0xFFFFFFFF; // bright white
            if ("media-playlist-shuffle-symbolic".equals(iconName) || "media-playlist-repeat-symbolic".equals(iconName) || "media-playlist-repeat-song-symbolic".equals(iconName)) {
                // Active shuffle/loop gets dynamic green color!
                iconColor = 0xFF34C759;
            } else {
                // Otherwise normal controls get light grey to white hover transition
                int val = (int) MathHelper.lerp(hoverProgress, 200, 255);
                iconColor = 0xFF000000 | (val << 16) | (val << 8) | val;
            }
    ...
    public boolean mouseClicked(double rx, double ry, int button, float width, float height) {
        ...
        if (currentMode == Mode.COMPACT) {
            ...
            // SECURITY CHECK: Only register the click if it is actually inside the capsule bounds!
            if (button == 0 && rx >= 0 && rx <= width && ry >= 0 && ry <= height) {
